...
contract('RntCrowdsale', function (accounts) {
    ...
    it("11 - Check that invest works", async function () {
        try {
            ...
            //Here we are adding minimal invest sum
            await crowdsale.setMinimalInvestment(web3.toBigNumber("5e19"));

            expectThrow(
                crowdsale.invest(uuid, {from: acc2, value: web3.toBigNumber("1000000000000000000")})
            );

            //Increase token price
            await pricing.setTokenPriceInWei(web3.toBigNumber("5e19"));

            await crowdsale.invest(uuid, {from: acc2, value: web3.toBigNumber("5e19")});

            assert.equal(
                (await token.balanceOf(acc2)).toString(),
                web3.toBigNumber("1e18")
            );

            expectThrow(
                crowdsale.invest(uuid, {from: acc2, value: web3.toBigNumber("4.9e19")})
            );
        } catch (err) {
            assert(false, err.message);
        }
